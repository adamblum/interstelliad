...
function createGame() {
    ...
    log(`Game created for ${playerCount} players. Waiting for players to join...`);
...
function joinGameAsPlayer() {
    ...
    // Check if planet is already taken
    if (gameState.joinedPlayers.some(p => p.planet === planet)) {
        alert('This planet has already been chosen by another player. Please select a different one.');
        return;
    }
    ...
    const playerId = gameState.joinedPlayers.length;
    gameState.joinedPlayers.push({
        id: playerId,
        name: playerName,
        planet: planet,
    ...
    log(`${playerName} joined from ${planet}!`);
    updateJoinedPlayersList();
    
    // Show start button if all players joined
    if (gameState.joinedPlayers.length === gameState.maxPlayers) {
        document.getElementById('startGame').style.display = 'block';
        log('All players have joined! Organizer can start the game.');
    }
}
...
function startGame() {
    ...
    gameState.players = gameState.joinedPlayers.map(jp => {
        const homePlanet = HOME_PLANETS[jp.planet];
        return {
            id: jp.id,
            name: jp.name,
            planet: jp.planet,
